...
def generate_user_report(
    ...
    # Build report
    lines = [
        f"# Profile Match Report: {user_id}",
        "",
        "## Your Profile",
        "",
        format_sections_for_report(user_sections),
        "",
        f"## Your Top {len(top_matches)} Matches",
        ""
    ]
    ...
    lines.extend([
        "## How to Reach Out",
        "",
        "When connecting with your matches:",
        "- Reference specific shared interests or complementary skills",
        "- Be genuine and specific in your outreach",
        "- Suggest concrete ways to collaborate or help each other",
        "- Follow up thoughtfully if you don't hear back initially",
        "",
        "---",
        "",
        f"*Report generated by prompt-mesh matching system*"
    ])
...
def generate_all_reports(
    ...
    for user_id in all_user_ids:
        ...
        # Generate report
        report_content = generate_user_report(
            user_id=user_id,
        ...
        # Save report
        report_file = outputs_path / f"{user_id}.md"
        report_file.write_text(report_content, encoding='utf-8')
    
    print(f"Saved {len(all_user_ids)} user reports to {outputs_path}")
    
    # Generate cohort summary
    cohort_summary = create_cohort_summary(all_edges, all_user_ids)
